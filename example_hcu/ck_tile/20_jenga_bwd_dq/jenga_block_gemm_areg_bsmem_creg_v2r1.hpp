...
struct JengaBlockGemmARegBSmemCRegV2R1
{
                                   ...
                                   const BBlockWindowTmp& b_block_window_tmp) const
    {
        ...
        if constexpr(a_block_tensor.get_thread_buffer_size() ==
                     ABlockTensorTmp::get_thread_buffer_size())
        {
            a_block_tensor.get_thread_buffer() = a_block_tensor_tmp.get_thread_buffer();
        }
        else
        {
            static_assert(a_block_tensor.get_thread_buffer_size() %
                              ABlockTensorTmp::get_thread_buffer_size() ==
                          0);
            static_for<0, a_block_tensor.get_thread_buffer_size(), 1>{}([&](auto i) {
                constexpr index_t src_i = i % ABlockTensorTmp::get_thread_buffer_size();
                a_block_tensor.get_thread_buffer()(i) =
                    type_convert<typename decltype(a_block_tensor)::DataType>(
                        a_block_tensor_tmp.get_thread_buffer()[number<src_i>{}]);
            });
        }
