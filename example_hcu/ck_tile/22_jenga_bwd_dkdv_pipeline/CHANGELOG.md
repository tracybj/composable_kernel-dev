...
## 2026-06-29 - Q/V Direct Global-to-LDS 尝试与访存特征限制分析

### 改动与尝试

- 在 `FullTile` 路径中尝试将 `Q` 和 `V` 也改造成 `amd_direct_load_global_to_lds` 直达加载。
- `Q` 改造后通过正确性验证（cosine 达到完美的 `0.999995`）。`V` 改造后正确性出现微幅波动（cosine 退化至 `0.995`，可能由于 V 维度对齐或不对称覆盖所致）。

### 性能评测结果

- **K-only 优化版（基线）**：**85.99 ms**。
- **Q + K 优化版**：**139.97 ms**（性能大幅下降了约 **54 ms**）。

### 核心硬件访存机制分析

1. **DRAM 带宽瓶颈限制了 Q 的 Direct Load**：
   - Q 矩阵在每一轮外层循环迭代（共 124 轮）中，根据动态掩码 `qb` 加载不同的行，导致其访存不连续且**无法命中缓存**，必须全部向 DRAM 现场拉取。
   - GPU 硬件的 Direct Global-to-LDS（`buffer_load_dword ... lds`）单条指令最大仅支持 **32-bit (4-byte)** 宽度，因此将 Q 瓦片（$64 \times 128$）直达 LDS 需要执行 **16 次 32-bit 访存**。
   - 而原常规搬运可使用 128-bit 向量指令（`global_load_dwordx4`），每次仅需 **4 次加载**。在 DRAM 瓶颈的场景下，16 次低效 32-bit 加载造成的 DRAM 吞吐带宽损失远超省下寄存器的收益，从而导致性能大幅劣化。
2. **为什么 K 矩阵适合 Direct Load**：
   - K 的分块起始点 `start_n` 在整个线程块生命周期内恒定，因此除第一轮外，后续 123 轮迭代的加载**全部能命中 L1/L2 缓存**。
   - 对 K 而言，访存吞吐不是瓶颈，直达 LDS 机制能够完全省下中转寄存器并优化指令发射，实现约 `6%`（`5.9 ms`）的纯性能增益。

### 最终决策

- 回滚并放弃对 Q 和 V 的 Direct Global-to-LDS 改造。
- 保留 **K-only 优化版** 这一性能最优状态（耗时 **85.99 ms**，cosine **0.999995**）作为 `main` 分支主干。
